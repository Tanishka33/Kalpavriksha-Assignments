...
int findTeamIndexByName(const char *name, const Team teams[]);
int updateTeamStats(Team *team);

int addPlayer(Team teams[]);
int displayTeamPlayers(const Team teams[]);
int displayTeamAvgScore(const Team teams[]);
int displayTopK(const Team teams[]);
int displayRoleOfTeam(const Team teams[]);
